...
\]
durch den induktiven Limes \thref{defi:kohomologiegruppe} auf einen Einschränkungshomomorphismus 
\[
...
\begin{defi}[Die Garben $\mathcal{H}^D_{D'}$]\thlabel{defi:produktgarbe}
	...
	\]
	Dann gilt für den Halm $\mathcal{H}^D_{D',x}=\mathcal{O}_{D',x}/\mathcal{O}_{D,x}$. Damit gibt es also einen Garbenepimorhpismus $\mathcal{O}_{D'}\rightarrow \mathcal{H}^D_{D'}$, dessen Kern $\mathcal{O}_D$ ist. Insgesamt gibt es eine exakte Sequenz
	\begin{center}
		\begin{tikzcd}
