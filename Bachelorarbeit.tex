...
\end{defi}
\nomenclature[Gm]{$f\mid U$}{Einschränkung der Funktion $f$ auf $U$, Notation in der Garbentheorie, \nomrefpage}
...
\begin{proof}
	...
	ein Isomorphismus ist für alle $\mathfrak{V}=(V_k)_{k\in K}$. Dann ist $\mathfrak{U}$ die feinste Überdeckung und es bleibt in \thref{defi:kohomologiegruppe} nur die Kohomologiegruppe von $\mathfrak{U}$. Nach \thref{lemma:verfeinerungsabbildung} bleibt noch Surjektivität zu zeigen.
...
\nomenclature[Ob]{$F^*$}{\nomrefpage}
 ...
 \begin{lemma}[Lemma von Dolbeault]\thlabel{satz:dolbeault}
...
\begin{lemma}\thlabel{lemma:hilbert}
Für eine offene Teilmenge $U\subset X$ einer Riemannschen Fläche $X$ ist $\mathcal{L}^2(U)$ ein Hilbertraum.
\end{lemma}
\begin{proof}
Sei $ D_r \coloneqq \{z\in D;~B(z,r)\subset D\}$ für ein $r>0$ Menge aller Punkte, deren Abstand zum Rand kleiner oder gleich $r$ ist. Sei für ein $f\in\mathcal{L}^2(D)$ die Taylorentwicklung um ein $a\in D_r$ durch
\[
f(z)=\sum_{n=0}^{\infty} c_n(z-a)^n
\]
gegeben. Dann gilt nach (\ref{bsp:norm}), dass
\[
|f(a)|^2=|c_0|^2\leq \sum_{n=0}\frac{|c_n|^2 r^{2n}}{n+1} = \frac{1}{\pi r^2}\norm{f}_{\mathcal{L}^2(B(a,r))}^2\leq \frac{1}{\pi r^2} \norm{f}_D,
\]
wobei $\norm{f}_D\coloneqq \sup\{|f(z)|;~z\in D\}$. Dann gilt also
\[
\norm{f}_{D_r}\leq \frac{1}{\sqrt{\pi}r}\norm{f}_D.
\]
Sei nun $(f_n)_{n\in \mathbb{N}}\in\mathcal{L}^2(D)$ eine Cauchy-Folge. Nach den Ungleichungen ist sie lokal gleichmäßig konvergent, damit ist die Grenzfunktion auch holomorph und natürlich integrierbar. Also ist sie in $\mathcal{L}^2(D)$ und der Raum ist vollständig.
\end{proof}
\nomenclature[As]{$B(a,r)$}{$=\{z\in\mathbb{C};~|z-a|<r\}$, die offene Kreisscheibe um $a\in\mathbb{C}$ mit Radius $r>0$}
\nomenclature[Os]{$\norm{f}_D$}{$=\sup\{|f(z)|;~z\in D\}$, die Supremumsnorm auf $D\subset\mathbb{C}$ offen}
Das folgende Lemma wird benötigt, um später eine wichtige Aussage für die Norm zu zeigen.
...
\end{proof}
Man kann nun die $\mathcal{L}^2$-Norm auch für Koketten mit Koeffizienten in der Garbe der holomorphen Funktionen definieren.
...
Direkt aus der Wahl der Norm folgt mit \thref{lemma:hilbert}:
\begin{koro}
    Die Räume $C^q_{\mathcal{L}^2}(\mathfrak{U},\mathcal{O})$ sind Hilberträume.
\end{koro}
\nomenclature[Al]{$\mathfrak{V}\ll\mathfrak{U}$}{$\mathfrak{V}$ ist Schrumpfung von $\mathfrak{U}$, \nomrefpage}
\nomenclature[Am]{$\mid \mathfrak{U}\mid$}{\nomrefpage}
...
Damit können wir die analoge Aussage zu \thref{lemma:normendlich} für integrierbare Kozykel treffen.
\newpage
...
\nomenclature[Ar]{$\mathrm{supp}(f)$}{Träger einer Funktion, \nomrefpage}
...
\]
Dieser ist wegen der vorigen Aussage auch ein Hilbertraum. Sei 
\[
...
\end{lemma}
\nomenclature[At]{$A\oplus B$}{Die direkte Summe der Vektorräume,Gruppen,Ringe,etc. $A$ und $B$}
\begin{proof}
Sei $\xi\in Z^1(\mathfrak{U},\mathcal{O})$ beliebig. Aus $\mathfrak{V}\ll \mathfrak{U}$ folgt nach \thref{schrumpfung}
\[
\norm{\xi}_{\mathcal{L}^2(\mathfrak{V})}\eqqcolon M <\infty.
\]
Wähle $C>0$ wie in (\ref{schrumpfung}) und $\varepsilon\coloneqq \frac{1}{2c}$. Dann gibt es nach \thref{norm} einen abgeschlossenen Untervektorraum $A\subset Z^1_{\mathcal{L}^2}(\mathfrak{U},\mathcal{O})$ mit endlicher Kodimension, sodass
\[
\norm{\xi}_{\mathcal{L}^2(\mathfrak{V})}\leq \varepsilon\norm{\xi}_{\mathcal{L}^2(\mathfrak{U})}.
\]
...
Es gibt nach \thref{schrumpfung} Elemente $\zeta_0\in Z^1_{\mathcal{L}^2}(\mathfrak{U},\mathcal{O})$ und $\eta_0\in C^0_{\mathcal{L}^2}(\mathfrak{W},\mathcal{O})$ mit
\[
...
2^{-n-1}M.
\]
Nach \thref{schrumpfung} gibt es wieder Elemente $\zeta_{n+1}\in Z^1_{\mathcal{L}^2}(\mathfrak{U},\mathcal{O})$ und $\eta_{n+1}\in C^0_{\mathcal{L}^2}(\mathfrak{U},\mathcal{O})$ mit
\[
...
\nomenclature[Ap]{$\mathfrak{U}\cap Y$}{\nomrefpage}
...
\end{koro}
\begin{proof}
    Falls wir $D'=D+P$ schreiben können, ergibt sich diese Aussage direkt aus \thref{koro:exakte kohomologiesequenz}. Sonst können wir $D'=D+P_1+\ldots+P_M$ schreiben für $P_i\in X$, somit folgt die Aussage per Induktion.
\end{proof}
...
\begin{koro}\thlabel{koro:ueberlagerung}
	...
	\end{koro}
\begin{proof}
	Die Abbildung aus \thref{satz:polstellenordnung} lässt sich als (nicht konstante) holomorphe Funktion $f:X\rightarrow \mathbb{P}_1$ auffassen (vgl. \cite[~S.8]{forster}), ist also eine Überlagerung der Zahlenkugel (vgl. \cite[~S.18]{forster}). Nach Satz 4.24 in \cite{forster} hat dann jeder Punkt die Vielfachheit $\leq g+1$, folglich ist die Blätterzahl $\leq g+1$.
...
\nomenclature[Oc]{$\chi(X)$}{Euler-Charakteristik, \nomrefpage}
...
\end{satz}
\begin{proof}
Nach \thref{koro:ueberlagerung} gibt es eine holomorphe Überlagerung $f:X\rightarrow \mathbb{P}_1$ mit $n\leq g+1$ Blättern. Da das kohomologische Geschlecht der Zahlenkugel $0$ ist, gilt nach der Riemann-Hurwitz-Formel (\ref{satz:riemann-hurwitz}), dass
\[
