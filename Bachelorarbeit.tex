...
\subsection{Einführung in die Garbentheorie}
Hier wird zunächst eine Einführung in die Theorie der Garben gegeben, welche sich mithilfe der Garbenkohomologie als nützliches Werkzeug für die Untersuchung der Riemannschen Flächen erweisen werden.
Diese wird im nächsten Abschnitt erläutert.  Motivierende Beispiele für die gegeben Definitionen lassen sich darunter finden.
...
\end{equation}
und das folgende Diagramm soll kommutieren
\newline
\begin{center}
\begin{tikzcd}
\mathcal{F}(U) \arrow{d}[swap]{\rho^U_V} \arrow{r}{\rho^U_W}
& \mathcal{F}(W) \\
\mathcal{F}(V) \arrow{ur}[swap]{\rho^V_W}
...
Wir schreiben für einen Schnitt $f\in\mathcal{F}(U)$ auch $p^U_V(f)=f \mid V$ und nennen die $p^U_V$ Einschränkungshomomorphismen. Außerdem schreibt man oft nur $\mathcal{F}$ statt $(\mathcal{F},p)$, wenn die Einschränkungshomomorphismen in natürlicher Weise gegeben sind (siehe dazu die Beispiele).
\end{defi}
...
\end{enumerate}
dann heißt $(\mathcal{F},p)$ \emph{Garbe}.
\end{defi}
\begin{bem}
...
Für $V\subset U $ offen sei $p^U_V: \mathcal{C}(U)\rightarrow \mathcal{C}(V)$ der übliche Einschränkungshomomorphismus. Dann ist $\mathcal C = (\mathcal C,p)$ eine Garbe, genannt Garbe der stetigen Funktionen auf X.
\item
Seien $X$ eine Riemannsche Fläche und und $\mathcal{O}(U)$ für $U\subset X$ offen die holomorphen Funktionen 
$f:U\rightarrow \mathbb{C}$. Mit dem üblichen Einschränkungshomomorphismus bildet auch $\mathcal{O}$ die Garbe der holomorphen Funktionen auf X.
Analog bildet $\mathcal{M}$ die Garbe der meromorphen Funktionen auf X und $\mathcal{E}$ die der stetigen.
\end{enumerate}
\end{bsp}

Die in dem Beispiel gegebenen Garben sind alles Garben von Vektorräumen bzw. Ringen. Als nächstes interessiert uns, wie die Elemente der $\mathcal{F}(U)$ in der Nähe eines Punktes $a\in X$ aussehen. Wir betrachten dazu die disjunkte Vereinigung
\[
\dot{\bigcup_{U\in \mathcal{T},a\in U}} \mathcal{F}(U).
\]

\begin{defi}[Halm,Keim]\thlabel{defi:halm}
Sei $\mathcal{F}$ eine Garbe auf dem topologischen Raum $X$. Zwei Elemente $f\in\mathcal{F}(U)$ und $g\in \mathcal{F}(V)$ für $U,V\subset X$ offen und $a\in U\cap V$ heißen äquivalent,
\[
f\sim_a g,
\]
...
\]
wobei letzeres der Ring der konvergenten Potenzreihen um $(z-a)$ mit komplexen Koeffizienten ist.
\end{bsp}
...
Seien $\mathcal{F},\mathcal{G}$ Garben abelscher Gruppen über dem topologischen Raum $X$. 
...
\subsection{Garbenkohomologie}
In diesem Abschnitt werden wir nun die Kohomologie von Garben erläutern, um die erste Kohomologiegruppe $H^1(X,\mathcal{F})$ für eine Riemannschen Fläche $X$ und eine Garbe abelscher Gruppen $\mathcal{F}$ zu definieren, das Objekt unserer Betrachtung in dieser Arbeit.
...
\[
C^q(\mathfrak{U},\mathcal{F}) \coloneqq \prod_{(i_0,...,i_q)\in I^{q+1}} \mathcal{F}(U_{i_0}\cap ...\cap U_{i_q}). 
\]
Die Elemente von $C^q(\mathfrak{U},\mathcal{F})$ heißen \emph{q-Koketten} und sind Familien
\[
(f_{i_0,...,i_q})_{i_0,...,i_q\in I}, \text{ sodass } f_{i_0,...,i_q}\in\mathcal{F}(U_{i_0}\cap ... U_{i_q})
\]
für alle $(i_0,...,i_q)\in I^{q+1}$.
Die Addition zweier Koketten ist komponentenweise definiert.
\end{defi}
...
\]
Wieder werden hier die Komponenten auf der rechten Seite eingeschränkt auf $U_i\cap U_j\cap U_k$.
\end{enumerate}
\vspace{0.5cm}
Diese Korandoperatoren bilden Gruppenhomomorphismen, da die Addition der q-Koketten Komponentenweise definiert war. Sei nun
\begin{align*}
...
\[
H^1(\mathfrak{U},\mathcal{F})\coloneqq \faktor {Z^1(\mathfrak{U}, \mathcal{F})}{B^1(\mathfrak{U}, \mathcal{F})} 
\]
...
\begin{align*}
t_{\mathfrak{V}}^{\mathfrak{U}}:& Z^1(\mathfrak{U},\mathcal{F})\rightarrow Z^1(\mathfrak{V},\mathcal{F})\\
&(f_{i,j})\mapsto (g_{k,l}),
\end{align*}
wobei 
\[
g_{k,l} \coloneqq  f_{\tau k,\tau l}\mid V_k\cap V_l
\]
für $k,l\in K$.
Folglich bleibt das Zerfallen eines Kozykels unter der Abbildung erhalten, das heißt mit der Quotientenabbildung  wird auch eine Abbildung 
\[
...
& =f_{\tau k,\tau l} +f_{\tau l,\tilde{\tau}k}-f_{\tau l,\tilde{\tau}k}-f_{\tau k,\tilde{\tau}l}\\ 
...
Für Injektivitiät zeigen wir:\\
Für einen Kozykel $(f_{i,j})\in Z^1(\mathfrak{U},\mathcal{F})$, dessen Bild unter der induzierten Abbildung in $Z^1(\mathfrak{V},\mathcal{•})$, muss auch der Kozykel selbst zerfallen.  Dann ist der Kern der induzierten Abbildung zwischen den Kohomologiegruppen $0$, also folgt Injektivität.\\
Sei also $f_{\tau k,\tau l}=g_k-g_l$ auf $V_k\cap V_l$, wobei $g_k\in \mathcal{F}(V_k)$. Es gilt dann nach Kozykelrelation auf $U_i\cap V_k\cap V_l$, dass
\[
...
\]
Dann kann man auf $(U_i\cap V_k)_{k\in K}$ die Garbeneigenschaft anwenden und erhält  ein $h_i\in \mathcal{F}(U_i)$ mit
\[
...
\]
Der Index $k$ war beliebig, also muss die obige Gleichung nach der Garbeneigenschaft auch auf ganz $U_i\cap U_j$ gelten. Damit zerfällt $(f_{i,j})$ auf $\mathfrak{U}$.
\end{proof}
Damit lässt sich nun eine Äquivalenzrelation auf der disjunkten Vereinigung aller Überdeckungen einführen, ähnlich zur Definition von \thref{defi:halm}.
\begin{defi}
	Zwei Kohomologieklassen $\xi\in H^1(\mathfrak{U},\mathcal{F})$ und $\eta\in H^1(\mathfrak{U'},\mathcal{F})$ heißen äquivalent ($\xi\sim\eta$), falls es eine offene Überdeckung $\mathfrak{U}$ mit $\mathfrak{V}\subset\mathfrak{U}$ und $\mathfrak{V}\subset \mathfrak{U'}$ gibt, sodass $t^{\mathfrak{U}}_{\mathfrak{V}}(\xi)=t^{\mathfrak{U'}}_{\mathfrak{V}}(\eta).$
...
\end{defi}
\begin{bem}
Wenn die Garbe aus Vektorräumen besteht, wie in dem hier betrachteten Fall $\mathcal{O}$, so bilden die Kokettengruppen auch Vektorräume. Damit genau so die Kozyklen- und Korändergruppen als Bild und Kern einer linearen Abbildung zwischen Vektorräumen. Folglich bildet auch die erste Kohomologiegruppe als Quotientengruppe auch einen Vektorraum. Dies ist vom großem Vorteil, denn schließlich lassen sich endlich-dimensionale Vektorräume deutlich leichter auf Isomorphie untersuchen oder unterscheiden als Gruppen.
\end{bem}
Für die folgenden Beispiele siehe (Forster, einfügen!).
\begin{bsp}\thlabel{bsp:kohomologie}
\renewcommand{\labelenumi}{(\roman{enumi})}
\begin{enumerate}
\item
$H^1(X,\mathcal{E})=0$, wobei mit $\mathcal{E}$ die Garbe der differenzierbaren Funktionen bezeichnet wird.
\item
$H^1(X,\mathbb{C})= 0$, wobei mit $\mathbb{C}$ die Garbe der konstanten Funktionen $X\rightarrow\mathbb{C}$ bezeichnet wird und $X$ einfach zusammenhängend ist.
\item
$H^1(X,\mathbb{Z})=0$ analog zu (ii).
\item Falls $X$ eine offene Kreisscheibe in $\mathbb{C}$ ist, gilt $H^1(X,\mathcal{O})=0$.
\end{enumerate}
\end{bsp}
Ein wichtiger Satz, der bei der Bestimmung der Kohomologiegruppen hilft, ist der folgende Satz, benannt nach dem Mathematiker Jean Leray.
...
\]
$\mathfrak{U}$ heißt dann \emph{Leraysche Überdeckung}
\end{satz}
\begin{proof}
	Wir zeigen, dass
...
\end{proof}
Die Definitionen lassen sich auch auf höher dimensionale Kohomologiegruppen fortführen, falls man Mannigfaltigkeiten von höherer Dimension betrachtet. Es ist aber auch möglich, die 0-te Kohomologiegruppe zu definieren.
...
\end{defi}
Dann gehört eine 0-Kokette $(f_i)_{i\in I}$ genau dann zu $Z^0(\mathfrak{U},\mathcal{F})$, wenn $f_i\mid U_i\cap U_j=f_j\mid U_i\cap U_j$ für alle $i,j\in I$. Mit der Garbeneigenschaft kann man diese Elemente zu einem globalen Element $f\in \mathcal{F}(X)$ fortsetzen, somit ergibt sich eine Isomorphie
...
\]
für Funktionen $f,g:U\rightarrow \mathbb{C}$. Mit einer Umformung analog zum Wirtinger Kalkül
...
\end{defi}
 \begin{defi}[Rücktransport von Differentialformen]
 	...
 	\]
 	Analog kann man diesen für Differentialformen 1. Ordnung definieren. Wir wissen aus den vorigen Definitionen, dass sich diese lokal als endliche Summe $\sum f_j\dif g_j$ schreiben lassen. Sei
 	\[
...
\end{defi}
Es folgt direkt, dass jede nicht konstante Abbildung zwischen Riemannschen Flächen eine Überlagerung ist.\cite[S.18]{forster}
\newpage
%-------------------------------------------------------------------------------------------------%
...
$$
\langle\psi_n,\psi_m \rangle = \iint\limits_B \psi_n(z)\overline{\psi_m(z)}\,\mathrm{d}x\,\mathrm{d}y = \int_0^{r}\int_0^{2\pi}
t^{n+m}e^{i(n-m)}t\,\mathrm{d}t\,\mathrm{d}\phi.
$$
Für $n\neq m$ verschwindet das Integral und für $n=m $ ergibt sich $$
...
$$
um den Punkt $a$. Dann gilt nach der Parsevalschen Gleichung 
$$
...
\end{bsp}
\begin{satz}
Sei $D\subset\mathbb{C}$ offen und $D_r \coloneqq \left\{
z\in \mathbb{C}: B(z,r)\subset \mathbb{D}\right\}$ die Menge aller Punkte in $D$, deren Distanz zum Rand kleiner oder gleich $r$ ist.
Dann gilt für $f\in \mathcal{L}^2(D,\mathcal{O})$, dass
$$
\norm{f}_{D_r} \leq \frac{1}{\sqrt{\pi}r}\norm{f}_{\mathcal{L}^2(D)},
$$
wobei $\norm{f}_{D_r} = \mathrm{sup}_{z\in D_r} |f(z)|$ die Supremumsnorm ist.
\end{satz}
\begin{proof}
Sei $a\in D_r$ und $f(z) = \sum_{n=0}^{\infty}c_n(z-a)^n$ die Taylorentwicklung von $f$ um $a$. Dann folgt mithilfe von Beispiel 3.1.1, dass
$$
|f(a)| = |c_0| \leq \frac{1}{\sqrt{\pi}r}\norm{f}_{\mathcal{L}^2(B(a,r))}\leq \frac{1}{\sqrt{\pi}r}\norm{f}_{\mathcal{L}^2(D)},
$$
wobei der letzte Schritt aus $B(a,r)\subset D $ folgt. Da $a\in D_r$ beliebig war, ist die Aussage bewiesen.
\end{proof}
...
Sei $X$ eine riemannsche Fläche und $\left( U_i^{*},z_i\right)_{1\leq i\leq n}$ eine endliche Familie von Karten, sodass $z_i(U_i^{*})\subset \mathcal{C}$ Kreisscheiben sind und seien die Mengen $U_i\subset U_i^{*}$ offen. Wir schreiben $\mathfrak{U}\coloneqq\left( U_i \right)_{1\leq i\leq n}$ und $\mathfrak{U}^{*}\coloneqq\left( U_i^{*}\right)_{1\leq i\leq n}$ .
\begin{enumerate}
\item
...
\end{defi}
\begin{bem}
Die Mengen $(U^*_i)_{1\leq i\leq n }$ müssen für diese Definition nicht $X$ überdecken.
\end{bem}
Dieses Lemma wird für den Beweis des darauffolgenden Lemmas, dem Schlüssel zu dem Endlichkeitssatz, benötigt.
...
T:\mathcal{L^2(D)}/A 	&\longrightarrow 	\mathbb{C}^{k\times n}\\
  f 					&\longmapsto 			     \begin{bmatrix}
													...
													\end{bmatrix}. \\
\end{align*} 
Diese ist wohldefiniert, da für $f,g\in \mathcal{L}^2(D)$ mit $f-g\in A$ gilt, dass $T(f-g)=0$, also $T(f)=T(g)$. Des weiteren ist die Abbildung injektiv , da per Definition $T(f)=0$ genau dann, wenn $f\in A$ und somit $f$ das neutrale Element im Quotientenraum definiert.\\ Damit ist Dimension des linken Raumes kleiner oder gleich des rechten, also $\leq kn$.
\end{proof}
...
\begin{lemma}
Seien $D' \subset D\subset \mathbb{C}$ offene Mengen und $D'$ relativ Kompakt in $D$ (Notation $D'\Subset D$). Für alle $\varepsilon >0$ gibt es einen abgeschlossenen Untervektorraum mit endlicher Kodimension $A\subset \mathcal{L}^2(D,\mathcal{O})$, sodass
$$
\norm{f}_{\mathcal{L}^2(D')} \leq \varepsilon \norm{f}_{\mathcal{L}^2(D)},\text{ für } f\in A.
$$
\end{lemma}
\begin{proof}
...
\item $D'\subset D\subset\bigcup_{j=1}^k B(a_j,r/2)$.
\end{enumerate}
...
\]
wobei $c_i\in\mathbb{C}$. Deshalb gilt nach \thref{norm}
\[
...
Dies ergibt
\[
\norm{f}_{\mathcal{L}^2(D')}\leq k2^{-n-1}\norm{f}_{\mathcal{L}^2(D)}\leq\varepsilon\norm{f}_{\mathcal{L}^2(D)}.
\]
